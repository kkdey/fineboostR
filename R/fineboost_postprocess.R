...
fineboost_get_csets <- function (ff,
  ...
  if (class(ff) == "fineboost"){
    ...
    for(rr in 1:nmf_try){
      Lmax = ff$Lmax
      set.seed(rr)
      res = NNLM::nnmf(ff$weights_path, k=Lmax, method ="scd", loss = "mkl")
      ...
      within_purity = c()
      for(ee in 1:length(evidence_strength)){
        within_purity = c(within_purity, min(abs(attr(X, "LD"))[confidence_sets[[ee]], confidence_sets[[ee]]]))
      }
      ...
      purity_score[which(tmp2 > min_between_LD)] = 0
      purity_score[which(within_purity < min_within_LD)] = 0

      confidence_sets_1 = confidence_sets[which(purity_score == 1)]
      ...
      ll = list("unfiltered_sets" = confidence_sets,
                ...
                "update_time" = sort(evidence_strength, decreasing = T),
